...
async function cds_bind(args, options, logger = LOG) {
    ...
    for (const resolvedService of resolvedServices) {
        ...
        const nameText = !isJavaProject && name && highlight(name);

        logger.info(['Binding', nameText, 'to', platformText(binding), bindingText(binding, highlight), kindText].filter(t => t).join(' '));
    ...
    logger.info();
    logger.info(`${info('TIP:')} Run with cloud bindings: ${bold(runCmd(options, isJavaProject))}`);
}
...
async function cds_bind_exec(command) { // eslint-disable-lint
    ...
    const bindingManager = new BindingManager({ env, output: 'info' })
