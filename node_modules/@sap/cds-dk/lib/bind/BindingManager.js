...
const { resolveBindings, getUnresolvedBindings } = require('./bindingResolver')
...
class BindingManager {
    ...
    async resolve(bindings) {
        ...
        return await resolveBindings({...this.options, bindings, cache: this.cache})
    }

    async bindingEnv(bindings) {
        if (!bindings) bindings = await this.resolve()
        if (bindings) {
            const VCAP_SERVICES = JSON.stringify(this.toVcapServices(bindings))
