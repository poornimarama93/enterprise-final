...
class UserCdsrcJsonStorageProvider {
    ...
    async ensureFileIsGitignored(file, logger = console) {
        ...
        logger.info(`adding entry '${file}' to ${GIT_IGNORE_FILE}.`);
...
`;
    ...
    async storeServices(services, options = {}) {
        ...
        //     if (!answer) {
        //         options.logger.info(`aborting cds bind; not writing any sensitive data to file [${jsonPath}]`);
        //         return;
        //     }
        // }
        ...
        if (options.profile) {
            options.logger.info(`Saving bindings to ${highlight(jsonPath)} in profile ${highlight(options.profile)}`);
        } else {
            options.logger.info(`Saving bindings to ${highlight(jsonPath)}`);
        }
    }
