...
function executeSync(options = {}, command, ...args) {
    ...
    if (acceptStatuses !== '*' && acceptStatuses.indexOf(result.status) === -1) throw new Error(`Command returned with status code ${result.status}`);
...
//    ctx.log(`exec: ${command} ${args.map( v => v.match(/\s/) ? `"${v}"` : v ).join(' ')}`);
    ...
    const result = await new Promise( (resolve, reject) => {
        ...
        const exit = status => {
            if (acceptStatuses !== '*' && acceptStatuses.indexOf(status) === -1) reject(new Error(`Command returned with status code ${status}`))
            else resolve({
                status,
