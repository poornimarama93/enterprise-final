...
module.exports = class ModelProviderService extends cds.ApplicationService {
  ...
  init() {
    ...
    this.on('getExtResources', async req => {
      ...
      try {
        let files = await Promise.all (rs.map (row => tar.xvz (Buffer.from(row.sources)) .to (dir))) // REVISIT: use pipe instead
        files = files.flat().filter (f => f.endsWith('.csv') || f.endsWith('.properties'))
        return await tar.cz (dir, files)
      } finally {
        await rimraf(dir)
      }
    })
